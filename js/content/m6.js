...
<p>A <code>MinHeap</code> class is provided (see “Provided code” above the editor) — use it, or start with sorting.</p>`,
  reasoning: [
    { cat: 'abstraction', q: 'This problem is two patterns glued together. Which two?', choices: ['Graph + BFS', 'Aggregate by key (Map), then select top K (heap or sort)', 'Sort + scan intervals', 'Set + binary search'], answer: 1,
      explain: '<p>Step 1 is Module 2’s totals map. Step 2 is the ranking. Naming the two layers separately is the “senior” answer.</p>' },
...
function topKAccounts(transactions, k) {
  ...
  hints: [
    '<p>Start with the Map of totals — exactly Exercise 2.2.</p>',
...
function topKAccounts(transactions, k) {
  ...
  followUp: {
    ...
    explain: '<p>The aggregation layer becomes long-lived state (Module 7 territory); the ranking layer runs per query. Separating the layers is what makes the change cheap.</p>'
  }
})}
`
      },
