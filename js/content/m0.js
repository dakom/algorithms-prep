...
<div class="card"><h4>🏗 Projects & 🎤 mock interview</h4><p>Module 6 evolves an in-memory ledger through five requirement changes in one editor — the "implement a small service" style interview. Module 7 is a timed, multi-part problem where the pattern is <em>not</em> announced.</p></div>
...
${widget('multi', {
  ...
  answers: [0, 2, 4],
  explain: '<p>Dependencies, referrals, and grid cells are all <em>entities connected to entities</em> — graphs. Duplicates → Set. Meetings → sort + scan. Top-5 → Map + heap/sort. The grid is the sneaky one: “not every graph looks like <code>{A: [B]}</code>” (Module 2 makes that concrete).</p>'
})}
...
<p>That paragraph is the template for every exercise from here on. Module 1 makes you type it.</p>
`
      },
      ...
      /* ------------------------------------------------ 0.3 ------ */
      {
        type: 'quiz',
        title: 'Quiz: the loop',
        intro: '<p>Five quick questions on process and recognition. 70%+ and move on — Module 1 starts the real work.</p>',
        questions: [
          {
          ...
          },
          {
            ...
            answer: 1,
            explain: '<p>“Group by” / “total by” is aggregation: one Map, one pass, accumulate. Module 1 drills the <code>(map.get(k) ?? 0) + amount</code> idiom.</p>'
          },
          {
