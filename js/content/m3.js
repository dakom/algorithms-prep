...
${code('js', 'example', `levels(tree)   // [["A"], ["B", "C"], ["D"]]   for the tree from 3.1`)}`,
  reasoning: [
    { cat: 'abstraction', q: 'Which graph traversal is “group by level”?', choices: ['DFS preorder', 'BFS — the queue naturally holds one level (plus part of the next) at a time', 'Inorder traversal', 'Sorting by depth'], answer: 1,
      explain: '<p>Exactly the BFS from Module 2, with the level boundary made explicit. (DFS with a depth parameter also works: <code>result[depth].push(val)</code> — a fine alternative.)</p>' },
...
function levels(root) {
  ...
  solutionExplain: '<p>Connect it back explicitly: this is Module 2’s BFS with the “distance rings” made visible. No visited set because a tree has no cycles — say that, it shows you know <em>why</em> the graph version needed one.</p>',
